...
class ShopListViewModel @Inject constructor(
    ...
    private val handler = CoroutineExceptionHandler { _, exception ->
        Log.d("ShopListViewModel", "CoroutineExceptionHandler got $exception")
    }
    ...
    fun crossOffShopListItem(itemId: Int) {
        viewModelScope.launch(ioDispatcher + handler) {
            shopListRepository.crossOffShopListItem(itemId)
        }
    }

    fun removeFromList(listId: Int, itemId: Int) {
        viewModelScope.launch(ioDispatcher + handler) {
            shopListRepository.removeFromList(listId, itemId)
        }
    }
    ...
    companion object {
        private const val DEFAULT_REFRESH_INTERVAL_MS = 3000L
    }
}